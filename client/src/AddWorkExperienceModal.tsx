...
import { addWorkExperience } from "./backendHelpers"
import type { NewWorkExperience, WorkSkill } from "./backendHelpers"
import { Context } from "./Context"

export default function AddWorkExperienceModal(props: {
  onClose: () => void
  onAdded: () => void
}) {
  ...
  const [positionZh, setPositionZh] = useState('')
  ...
  const handleSubmit = async () => {
    setSubmitting(true)
    try {
      const payload: NewWorkExperience = {
        company: company.trim(),
        company_zh: companyZh.trim(),
        position: position.trim(),
        position_zh: positionZh.trim(),
        period: period.trim(),
  ...
  return (
    <div className="fixed inset-0 z-50 flex min-h-dvh w-full flex-col gap-4 overflow-y-auto bg-[#FFD6C9] p-6">
      <div className="flex flex-col w-full items-center">
        <h1 className="text-2xl font-semibold text-black">{isChinese ? '添加工作经历' : 'Add Work Experience'}</h1>
        <p className="mt-1 text-sm text-black/70">{isChinese ? '请填写您的工作经历详情。' : 'Add your work experience details.'}</p>
      </div>
      <div className="mx-auto flex w-full max-w-2xl flex-col gap-4">

        <div className="grid min-w-0 grid-cols-2 gap-4 [&>*]:min-w-0">
          <Input label={isChinese ? '公司 (英文)' : 'Company (English)'} type="text" value={company} onChange={(e) => setCompany(e.target.value)} />
          <Input label={isChinese ? '公司 (中文)' : 'Company (Chinese)'} type="text" value={companyZh} onChange={(e) => setCompanyZh(e.target.value)} />
          <Input label={isChinese ? '职位 (英文)' : 'Position (English)'} type="text" value={position} onChange={(e) => setPosition(e.target.value)} />
          <Input label={isChinese ? '职位 (中文)' : 'Position (Chinese)'} type="text" value={positionZh} onChange={(e) => setPositionZh(e.target.value)} />
          <Input label={isChinese ? '时间段 (英文)' : 'Period (English)'} type="text" value={period} onChange={(e) => setPeriod(e.target.value)} />
